...
namespace HTTPClientConsoleApp
{
    ...
    public class InfluxDBNet5Client : IInfluxDBNet5Client
    {
        ...
        public object Request(string url, string method, object data = null)
        {
            ...
            var result = GetResult(url, method, data);
            return result.Result;
        }

        public async Task<object> GetResult(string uri, string method, object data)
        {
            ...
            if (method == "GET")
            {
                resStream = await httpClient.GetStreamAsync(uri);
            ...
            else
            {
                ...
                if (data != null)
                {
                    postBody = JsonConvert.SerializeObject(data);

                }
                HttpResponseMessage wcfResponse = await httpClient.PostAsync(uri, new StringContent(postBody, Encoding.UTF8, "application/json"));
                HttpContent Res = wcfResponse.Content;
                resStream=await Res.ReadAsStreamAsync();
                
                //await wcfResponse.Content.CopyToAsync(resStream);
            }
